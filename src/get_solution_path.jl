...
function get_solution_path!(parameters::DataFrame)
    ...
    for t in range(1, length(parameters.t_delivery) - 1)
        normalized_delivery_size_at_stake_k = parameters.k_stock[t] / parameters.N[1]
        ...
        X_C = get_vaccine_stock_coverage(current_inventory_size, parameters)
        a_t = get_vaccine_action!(X_C, next_delivery_time, parameters)
        ...
        prior_inventory = x_t_0_k.K_stock
    end
