...
- `DataFrame`: A DataFrame containing the initial state of the system, with the following columns:
  ...
  - `"X_0_mayer"`: Initial value for the Mayer state variable.
  - `"K_stock"`: Normalized stock size after the first delivery.
...
- The stock size (`k_0`) is normalized by dividing the initial stock size (`k_stock`) by the total population size (`N`).
"""
